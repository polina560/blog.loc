...
use CKSource\CKFinder\Exception\{InvalidConfigException, InvalidResourceTypeException};
use DateTime;
use Exception;
...
class Config
{
    ...
    {
        $checkMissingNodes = static function (array $required, array $actual, $prefix = '') {
        ...
        $requiredRootNodes = [
            'authentication', 'licenseName', 'licenseKey', 'privateDir', 'images', 'backends', 'defaultResourceTypes',
            'resourceTypes', 'roleSessionVar', 'accessControl', 'checkDoubleExtension', 'disallowUnsafeCharacters',
            'secureImageUploads', 'checkSizeAfterScaling', 'htmlExtensions', 'hideFolders', 'hideFiles', 'forceAscii',
            'xSendfile', 'debug', 'pluginsDirectory', 'plugins',
        ];

        $checkMissingNodes($requiredRootNodes, $this->options);
        $checkMissingNodes(
            ['backend', 'tags', 'logs', 'cache', 'thumbs'],
            $this->options['privateDir'],
            '[privateDir]'
        );
        ...
        foreach ($this->options['resourceTypes'] as $i => $resourceTypeConfig) {
            ...
            if (!in_array($resourceTypeConfig['backend'], $backends, true)) {
                throw new InvalidConfigException(
                    "Backend '{$resourceTypeConfig['backend']}' is not defined: [resourceTypes][$i]"
                );
            }
        }
    ...
    protected function process(): void
    {
        ...
        $formatToArray = static function ($input) {
     ...
     */
    public function getDefaultResourceTypes(): array
    {
     ...
     */
    public function getResourceTypeNode(string $resourceType): array
    {
     ...
     */
    public function getHideFilesRegex(): string
    {
        static $hideFilesRegex;

        if (!isset($hideFilesRegex)) {
            $hideFilesConfig = $this->options['hideFiles'];
            $hideFilesRegex = $this->getHideRegex($hideFilesConfig);
        }
     ...
     */
    public function getHideFoldersRegex(): string
    {
        static $hideFoldersRegex;

        if (!isset($hideFoldersRegex)) {
            $hideFoldersConfig = $this->options['hideFolders'];
            $hideFoldersRegex = $this->getHideRegex($hideFoldersConfig);
        }

        return $hideFoldersRegex;
    }

    private function getHideRegex(array|string|null $hideConfig = null): string
    {
        if ($hideConfig && is_array($hideConfig)) {
            $hideRegex = implode('|', $hideConfig);
            $hideRegex = strtr($hideRegex, ['?' => '__QMK__', '*' => '__AST__', '|' => '__PIP__']);
            $hideRegex = preg_quote($hideRegex, '/');
            $hideRegex = strtr($hideRegex, ['__QMK__' => '.', '__AST__' => '.*', '__PIP__' => '|']);
            $hideRegex = '/^(?:' . $hideRegex . ')$/uim';
        } else {
            $hideRegex = '';
        }
        return $hideRegex;
     ...
     */
    public function extend(string $nodeName, array $values): void
    {
     ...
     */
    public function getPrivateDirPath(string $privateDirIdentifier): mixed
    {
     ...
     */
    public function isDebugLoggerEnabled(string $loggerName): bool
    {
     ...
     */
    public function getBackendNode(string $backendName): array
    {
        ...
        throw new InvalidArgumentException(
            sprintf('Backend %s not found. Please check configuration file.', $backendName)
        );
    }
}
